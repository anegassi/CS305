...
const cal = myExports.calculator;  //do this for all of the functions used in the Mocha tests




describe("test calculator with values 2, 3", function () {
    cal.setValues(2, 3);  //values for the tests

    it("checks initial values 2 and 3", function () {
        assert.strictEqual(cal.a, 2);
        assert.strictEqual(cal.b, 3);
    });

    it("when 2 and 3 are entered, the sum is 5", function () {
        assert.strictEqual(cal.sum(), 5);
    });

    it("when 2 and 3 are entered, the product is 6", function () {
        assert.strictEqual(cal.mul(), 6);
    });
